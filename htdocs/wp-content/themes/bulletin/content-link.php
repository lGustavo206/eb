...
if ( is_singular() && is_main_query() ) {
	...
	if( of_get_option('blog_single_thumbnail', '1') == '1' && has_post_thumbnail() ) { ?>
            <div id="post-thumbnail">
                <a href="<?php echo get_post_meta( get_the_ID(), 'wpex_post_url', true); ?>" title="<?php the_title(); ?>" target="_blank"><img src="<?php echo aq_resize( wp_get_attachment_url( get_post_thumbnail_id() ),  wpex_img( 'post_width' ), wpex_img( 'post_height' ), wpex_img( 'post_crop' ) ); ?>" alt="<?php echo the_title(); ?>" /></a>
            </div><!-- /post-thumbnail -->
	<?php }
...
*****************************************************/
    ...
    <article <?php post_class('post-entry grid-3 clearfix'); ?>>  
        ...
        <div class="post-entry-details">
        	<h3><a href="<?php echo get_post_meta( get_the_ID(), 'wpex_post_url', true); ?>" title="<?php the_title(); ?>" target="_blank"><?php the_title(); ?></a></h3>
